...
- **Frontend Live on Vercel**: `https://sih-project-mocha-zeta.vercel.app` (or custom Vercel domain)
- **Architecture**: Hybrid Next.js 14 App Router (Frontend) + FastAPI / PyTorch ML Service (Backend)

---

## ⚡ Vercel Deployment Configuration (Crucial)

This repository is structured as a monorepo. The Next.js web application is located in the `voiceshield-ai/frontend` subdirectory.

### 2-Step Configuration on Vercel Dashboard
...
2. **Trigger Build / Redeploy**:
   ...
   - Vercel will automatically detect **Next.js**, run the build, and publish the live site.
...
1. Deploy `voiceshield-ai/backend` on Render, Railway, or Fly.io (Docker or Python 3.11).
...
```powershell
# In voiceshield-ai/frontend:
bun install   # or npm install
bun run dev   # Runs at http://localhost:3000

# In voiceshield-ai/backend:
python -m venv .venv
